...
llm = ChatOllama(model=OLLAMA_MODEL, base_url=OLLAMA_BASE_URL, temperature=0.3, num_predict=512)
...
class RAGGraphNodes:
    ...
    async def search_sheets(self, state: RAGState) -> RAGState:
        ...
        try:
            ...
            for result in results:
                event = result['event']
                point = result['point'][:500]  # 500文字に拡張
    ...
    async def generate_answer(self, state: RAGState) -> RAGState:
        ...
        max_context_length = 2500  # 2000→2500に拡張
...
【回答】"""
        ...
        try:
            ...
            state['answer'] = f"⚠️ 回答生成エラー: {str(e)}"
            state['answer_quality'] = "insufficient"
...
async def on_ready():
    """Bot起動時の処理"""
    logger.info(f'{bot.user} としてログインしました（LangGraph強化版）')
    logger.info(f'Ollama Model: {OLLAMA_MODEL}')
    logger.info(f'Embedding Model: {OLLAMA_EMBEDDING_MODEL}')
    
    try:
        test_response = await asyncio.to_thread(lambda: llm.invoke("テスト"))
...
@bot.command(name='ask', help='RAGシステムに質問します（LangGraph強化版）')
async def ask_command(ctx, *, question: str):
    ...
    embed.set_footer(text=f"{metrics} | Powered by LangGraph + Ollama")
...
async def stats_command(ctx):
    ...
    embed.add_field(name="🤖 使用モデル", value=f"LLM: {OLLAMA_MODEL}\nEmbed: {OLLAMA_EMBEDDING_MODEL}", inline=False)
