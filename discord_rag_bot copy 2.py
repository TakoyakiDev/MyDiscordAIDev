...
from langchain_ollama import ChatOllama, OllamaEmbeddings  # ← 変更
...
class RAGSystem:
    ...
    async def _get_sheets_context(self, query: str) -> Dict:
        ...
        try:
            results = await self.sheets_retriever.semantic_search(query, top_k=3)  # 5→3に削減
            context = ""
            sources = []
            
            for result in results:
                event = result['event']
                point = result['point'][:300]  # 500→300に削減
...
async def ask_command(ctx, *, question: str):
    ...
    embed = discord.Embed(
        ...
        color=discord.Color.green()  # Ollama使用時は緑色
    )
